...
public class UserServices implements IUserService {
    ...
    @Override
    public boolean login(String password, String passwordHash, LogInfor logInfor) {
        if (BHash.login(password, passwordHash)) {
            Log log = new Log();
            log.setIpAddress(logInfor.getIpAddress());
            log.setAddress("Login");
            log.setLogLevel(logInfor.getLogLevel());
            log.setCurrentValue("");
            log.setUpdatedValue("");
            logDAO.save(log);
            return true;
